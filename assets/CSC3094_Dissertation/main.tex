...
\date{May 2026, Newcastle University}
...
Each acronym is introduced in full on first use; this table collects them for quick reference.
...
F1 score & Harmonic mean of precision and recall, the binary classification metric \\
...
HIPAA & Health Insurance Portability and Accountability Act (US, 1996; federal healthcare data privacy law used here as the prototype domain-specific privacy statute) \\
...
To meet this aim, the project pursues five SMART (Specific, Measurable, Achievable, Relevant, Time-bound) objectives.
...
For the AML task itself, the IBM AML dataset of Weber et al.~\cite{weber2018scalablegraphlearningantimoney} provides a synthetic yet structurally realistic corpus of interbank transactions, partitioned by originating bank and annotated with account-level laundering labels. The synthetic provenance is both a strength and a limitation: it sidesteps the regulatory hurdles that would accompany real banking data and enables open, reproducible research, but it does not fully capture the adversarial creativity of real-world money launderers, whose strategies evolve to evade detection. The dataset's account-level labels align with the F1 score as the natural utility metric, following the evaluation tradition formalised in the MUC (Message Understanding Conference) series~\cite{10.3115/1072064.1072067}.

DeepSeek-R1-Distill-8B supplies a reasoning-capable backbone suited to the chain-of-thought investigative style required by AML triage; the IBM AML dataset supplies a domain-appropriate, openly available benchmark against which that capability can be measured.


\chapter{Methodology}
\label{ch:method}

This chapter presents the design, implementation, and evaluation configuration of the privacy-preserving federated LLM architecture. Three domain concepts underpin every subsequent design decision and are defined here for precision before the requirements and architecture are stated.

\textit{Cross-silo Federated Learning} is the variant of FL in which a small number of well-identified, contractually bound institutions (banks, hospitals, law firms) each hold a partition of the data and want to learn a shared model without exchanging records. It is distinct from \textit{cross-device} FL, where the participants are millions of unidentified end-user devices. Cross-silo participants are usually assumed to follow the protocol and have substantial on-site compute; cross-device participants are usually assumed to be unreliable and intermittently connected.

\textit{Anti-Money Laundering (AML)} is the regulatory and analytic discipline by which financial institutions detect and report transactions consistent with money laundering or terrorist financing. The core computational task is binary classification on graph-structured data: each account is labelled \textit{clean} or \textit{suspicious}, and the analytic signal sits in the multi-hop transaction structure rather than in any individual transaction. Real banks file \textit{Suspicious Transaction Reports} (STRs) under FATF Recommendation~20 and its national transpositions; STR generation is treated here as a downstream consequence of correctly classifying suspicious accounts.

\textit{The privacy boundary} is the architectural construct that specifies which artefacts may leave a bank node. Raw transactions, account identifiers, graph queries, and any statistic derived from raw transactions remain inside the boundary. The only artefact that crosses it is an encrypted low-rank adapter weight delta. Critically, the boundary is enforced by code structure rather than policy: the modules with raw-data access are entirely absent from the federated communication path.

\section{Requirements}
\label{sec:meth:requirements}

The following requirements are derived from the five SMART objectives of Section~\ref{ch:introduction}.
...
Directive (EU) 2018/1673, the Sixth Anti-Money Laundering Directive~\cite{eu6amld2018}, requires Member States to make the main money laundering offences punishable by a maximum term of imprisonment of at least four years (Article~5(2)), lists 22 predicate offences that all member states must criminalise, and extends criminal liability to legal persons. The Directive entered into force on 3~December 2020, with transposition required by 3~June 2021.
...
\addlinespace
GDPR Art.~25~\cite{gdpr2016} & Data protection by design and by default: technical measures integrating data-protection principles into processing itself & Module layout segregates raw-data access (data ingestion and graph store modules) from the federation path (the federation module); the default is non-disclosure & Met by design \\
\addlinespace
...
\addlinespace
6AMLD~\cite{eu6amld2018} & Maximum sentence of at least 4 years (Art.~5(2)); corporate criminal liability; 22 predicate offences & Asymmetric-loss framing throughout: peak and final F1 score reported separately, recall-driven cost analysis in Section~\ref{sec:results:cost}, MIA limits acknowledged & Acknowledged (regulatory backdrop) \\
\bottomrule
\end{tabularx}
\end{table}
...
The codebase is organised into six top-level modules, each with a single responsibility.

\begin{itemize}
    \setlength\itemsep{0em}
    \item The data ingestion module ingests the IBM AML CSV, partitions it by originating bank, and produces stratified 70/15/15 train/validation/test splits per partition.
    \item The graph module contains the \texttt{GraphStore} abstract interface, three concrete backends (Kuzu, NetworkX, Neo4j), a factory, and the bank-scoped pattern detection module.
    \item The model module loads the DeepSeek-R1-Distill-Llama-8B backbone under 4-bit QLoRA quantisation and attaches the LoRA adapter.
    \item The pipeline module assembles the prompt for a target account, runs greedy generation against the adapted model, and parses the verdict from the model's reply.
    \item The federation module contains the in-process federated client (fit, evaluate, MIA, with a Flower-compatible interface), the aggregation strategies (FLoRA and FedAvg), and the centralised baseline runner.
    \item The security module implements the symmetric encryption wrapper used to protect adapter deltas in transit.
\end{itemize}
...
The graph module defines an abstract \texttt{GraphStore} interface with four operations: \texttt{ingest(transactions)}, \texttt{retrieve\_context(account\_id, mode)}, \texttt{structural\_signals(account\_id)}, and \texttt{close()}. Three concrete classes implement this interface:
...
A factory function selects the concrete backend based on a single \texttt{Config.graph\_backend} string. The federation and RAG layers depend only on the abstract interface; they will continue to work if a new backend (for example, TigerGraph or ArangoDB) is added later. Figure~\ref{fig:arch-strategy-pattern} shows the resulting class structure. This is exactly the kind of structural adaptability called for by the project instruction that the architecture must be reusable in practice beyond the dissertation.
...
The bank-scoped pattern detection module translates structural graph queries into named, LLM-digestible AML typologies. Five patterns are detected:
...
The federation client module implements an in-process federated client (mirroring Flower's client interface but not using the Flower runtime, for the reasons described in Section~\ref{sec:bg:fl}) whose \texttt{fit} step is responsible for one local epoch of training, and whose \texttt{evaluate} step is responsible for both the utility metric and the membership inference AUC.

Training uses AdamW with learning rate $2 \times 10^{-4}$ and a batch size of one, the latter imposed by the GPU memory budget of a single A100 instance that also holds the 4-bit backbone, the adapter, and the Kuzu database handle. Each round samples 100 accounts from the client's training partition, forced to a 50/50 suspicious/clean ratio through oversampling with replacement of the minority class (this step is necessary because the raw class imbalance of 1-2 per cent makes unbiased sampling produce rounds with zero positives). Evaluation runs against each client's full stratified test split, yielding between 380 and 726 accounts per client (averaging 578). Gradient accumulation is not enabled in the benchmark and is a future-work item.
...
Three aggregation strategies are implemented in the federation module:
...
\input{res/tab-config-summary}

\subsection{Evaluation criteria}
\label{sec:meth:eval-criteria}

Three evaluation criteria are reported, one per axis of the trilemma. All three are computed per round per client and aggregated across clients for the figures in Chapter~\ref{ch:results}; the per-client values are also retained in the archived JSON for downstream inspection.
...
The benchmark in Chapter~\ref{ch:results} is a single-GPU simulation. This section translates the simulation's measurements into the figures a deployment engineer would need in order to put the architecture into a real cross-silo consortium: bandwidth on a real WAN link, GPU-hour budget under typical cloud pricing, the audit artefacts the model-risk regimes of Section~\ref{sec:meth:regulatory} require, and the operational concerns (key management, rollback) that the simulation deliberately abstracts away. All numerical values reported in this section are produced by a dedicated verification script in the public repository; the dissertation cites the script's outputs.

\subsection{Bandwidth on a real WAN}

The Fernet-wrapped FLoRA adapter delta measures $18{,}197{,}432$~bytes per client per round, equivalent to $145.58$~megabits. On a $100$~Mbps inter-datacentre link this transfers in $1.46$~seconds, on a $1$~Gbps link in $0.15$~seconds, and on a $10$~Gbps link in $0.015$~seconds. The mean fit-stage wall clock for the same client per round is $770.1$~seconds (FLoRA-with-RAG, archived benchmark). The compute-to-communications ratio is therefore approximately $530\times$ at $100$~Mbps, $5{,}290\times$ at $1$~Gbps, and $52{,}900\times$ at $10$~Gbps. This is the operational consequence of the bandwidth axis collapsing by nearly three orders of magnitude (Section~\ref{sec:results:efficiency}): once the federated payload is the size of a small adapter rather than the size of the backbone, network choice ceases to be a deployment constraint; the wall-clock budget is set entirely by GPU compute.

\subsection{GPU-hour budget at on-demand cloud pricing}

A full ten-round, three-client session of FLoRA-with-RAG consumes $6.42$~A100-hours at the fit stage (the dominant stage; eval, MIA, and aggregation account for under $5$~per cent of round wall clock). At publicly listed on-demand A100 pricing the per-session cost falls between USD~$6.87$ at the specialised-cloud rate of USD~$1.07$ per A100-hour and USD~$26.25$ at the AWS (Amazon Web Services) p4d effective rate of USD~$4.09$ per A100-hour. At a weekly retraining cadence the consortium consumes $334$~A100-hours per year, costing between USD~$357$ and USD~$1{,}365$ at the same range. These figures are deliberately quoted per consortium and per session rather than per bank, since the federated cost is dominated by the per-client fit stage and does not scale with the size of the aggregator. They are also meaningful in absolute terms: an annual training budget of less than two thousand US dollars is small enough that compute is not the constraint on whether a consortium of mid-sized banks can run this architecture.

\subsection{Inference cost at production scale}
\label{sec:meth:inference-cost}

The training cost estimated above covers federated retraining. A production deployment also incurs \textit{inference} cost: running the fine-tuned model to score flagged accounts at monitoring time. The following estimates are produced by a dedicated verification script in the public repository and are framed as order-of-magnitude projections, not measurements; actual figures depend on hardware, batching strategy, and contract pricing.

Three modelling choices are assumed. First, a rules-based pre-filter (standard in AML transaction monitoring) flags approximately $5$\% of accounts for LLM review; the LLM therefore scores accounts, not every transaction, and the per-transaction cost is the per-account cost diluted by the flag rate. Second, each LLM call is estimated at $3{,}000$ input tokens (a $30$--$90$~day transaction list plus RAG structural signals) and $250$ output tokens (verdict plus brief reasoning). Third, the self-hosted compute cost uses the same A100 on-demand pricing range as the training estimate (USD~$1.07$--$4.09$ per A100-hour).

For the $8$-billion-parameter benchmark model (DeepSeek-R1-Distill-Llama-8B at 4-bit NF4), single-GPU throughput of approximately $150$~tokens per second yields an inference cost of approximately USD~$0.016$ per account at mid-range pricing, corresponding to USD~$\approx 16$ per million transactions monitored. Scaling to a production-grade $70$-billion-parameter model (e.g.\ Llama-3.1-70B at 4-bit on a single A100 80~GB at approximately $22$~tokens per second) raises the per-account cost to approximately USD~$0.106$, or USD~$\approx 106$ per million transactions. These estimates assume sequential single-call inference; batched production throughput would reduce per-call cost in proportion to batch utilisation.

For context, frontier API models (GPT-4o at USD~$2.50$/$10.00$ per million input/output tokens; Claude~3.5~Sonnet at USD~$3.00$/$15.00$) fall in the USD~$2$--$13$ per million transactions range. These are listed for reference only: API models cannot participate in FLoRA federated fine-tuning and transmit account data to a third-party server, making them architecturally incompatible with the privacy objective of this work.

In all self-hosted cases the per-transaction inference cost is two to three orders of magnitude below industry-survey estimates of the alert-investigation cost (USD~$30$--$70$ per alert~\cite{lexisnexis2024costukfc}). Inference compute is therefore not the economic constraint on adoption; investigator capacity is.
...
The repository contains small helper scripts that exercise the federation loop for a single round against a miniature partition, to catch integration-level regressions between the data layer, the graph layer, and the model layer. These are not a substitute for a full benchmark run, but they reliably catch issues such as tokeniser-artefact regressions (for example, the \enquote{fast tokeniser produces Ġ/Ċ artefacts} problem that is guarded against by the \texttt{Config.use\_fast\_tokenizer=False} default).

\subsection{Benchmark reproducibility}

The formal benchmark presented in Chapter~\ref{ch:results} was executed on 2~April~2026 and is archived in the project repository. Twelve JSON files are committed (six \texttt{config\_*.json}, six \texttt{history\_*.json}), corresponding to the six-condition cross of three aggregators and two retrieval modes. The exact code state that produced those numbers is pinned by the annotated git tag \texttt{benchmark-2026-04-02}, which points at commit \texttt{01a6cce}. A reviewer reproducing the results can therefore execute \texttt{git checkout benchmark-2026-04-02} and replay the notebook against the archived configurations.

\subsection{Limitations of the testing strategy}

Two limitations apply. First, the benchmark is single-seed, so only per-round variance - not cross-seed variance - is reported. Second, pattern detection is verified against hand-constructed graphs, but the model's verdict cannot be verified as semantically correct beyond the supervised label.

\section{Threat Model}
\label{sec:meth:threat}

The privacy claims in this chapter are bounded by the adversary model below, not by it in absolute terms. The architecture is designed to resist five adversary classes representative of a cross-silo AML deployment; it offers no guarantees against an active byzantine adversary, and that exclusion is explicit.

\subsection{Honest-but-curious aggregator}

\textit{Capability.} The aggregation server follows the protocol exactly but inspects every received adapter delta and every aggregated global adapter. This is the standard cross-silo FL threat model.

\textit{Defence.} Adapter deltas are Fernet-encrypted in transit (Section~\ref{sec:meth:impl}), providing transport-layer confidentiality against a passive network observer; this is not a formal ML privacy guarantee, as the aggregation server receives the plaintext delta after decryption. The substantive protection is structural: the payload is a rank-$r$ projection of the gradient signal, which limits recoverable information relative to full-parameter updates. The plaintext content of a delta is, by construction, the gradient signal of an arbitrary low-rank transformation of the local supervisory loss; it does not encode raw transactions. The membership-inference experiments in Section~\ref{sec:results:privacy} show that a threshold-based attacker observing the resulting model cannot meaningfully distinguish training accounts from test accounts.

\textit{Residual risk.} A stronger attacker - shadow-model construction or gradient inversion against low-rank deltas - could recover more information. Stronger attacks are a future-work item.
...
\textit{Residual risk.} A majority of malicious clients would arbitrarily steer the aggregated update; the architecture provides no defence. A deployment with weaker trust assumptions would substitute a Byzantine-robust aggregator.
...
\textit{Defence.} Each bank's graph store, pattern detector, and local adapter exist only inside that bank's privacy boundary. Other participants' raw records and structural signals are not present at any point in the local node's filesystem or process memory. The only cross-institution artefact locally readable is the global adapter - the same low-rank transformation discussed above.
...
\textit{Residual risk.} The observer learns the timing of communication rounds, which could be informative in tightly synchronised deployments. The architecture does not defend against traffic-analysis attacks, a defence is out of scope here.
...
\textit{Residual risk.} A well-resourced attacker with access to training metadata could mount stronger reconstruction attacks than the threshold-based probe used here; the architecture makes no formal differential-privacy claim.
...
The chapter reports the six-condition benchmark from Chapter~\ref{ch:method}: three aggregators (centralised, FedAvg, FLoRA) crossed with two retrieval modes (flat, RAG), ten rounds per condition, three bank clients per round.

\textit{What was measured?} Section~\ref{sec:meth:config} (Methodology) states the experimental configuration and defines the three evaluation criteria (F1 score, communication volume plus per-stage wall-clock, membership-inference AUC).
...
The chapter closes in Section~\ref{sec:results:limits} with a self-audit of the limitations of the evaluation, which the conclusions chapter then picks up as future-work items.

Every figure is produced from the archived JSON files at git tag \texttt{benchmark-2026-04-02} by the figure-generation scripts in the public repository (\url{https://github.com/SlaviXG/thinking-inside-the-box}), and is reproducible from the pinned commit \texttt{01a6cce}. Numerical claims in the prose, including the cost projections of Section~\ref{sec:results:cost} and the production-deployment figures cross-referenced from Chapter~\ref{ch:method}, are re-derived independently by the verification scripts in the public repository.

\section{Utility: F1 Score Across Conditions}
\label{sec:results:utility}

\input{res/tab-f1-per-round}
...
\input{res/tab-ablation-flat-vs-rag}

\section{Operational Cost: Projecting the Trilemma onto Investigation Budgets}
\label{sec:results:cost}

The utility, efficiency, and privacy axes reported above are intrinsic measurements of the model. The operational consequence at deployment time is dominated by a different number: how many alerts an analyst has to investigate per period, and what fraction of those are false. AML investigations are widely estimated to cost USD~30-70 per alert, with 85-95\% of transaction-monitoring alerts resolving as false positives; these are headline figures from industry compliance surveys~\cite{lexisnexis2024costukfc} rather than independently audited values, and the limitations of this sourcing are acknowledged in Section~\ref{sec:results:limits}. This subsection projects the benchmark's measured precision and recall onto a reference monitoring volume of one million transactions, holding the dataset's empirical positive-class rate of 1.5\% fixed, so that the trilemma trade-offs can be read in investigator-budget units rather than F1-score units. The full computation is performed by the cost-of-misclassification verification script in the public repository; the dissertation reports its outputs.
...
\begin{table}[t]
\centering
\caption[Projected investigation cost per million transactions]{Projected operational cost per million monitored transactions, at the peak F1 score round of each condition. \texttt{FP/M} and \texttt{FN/M} are projected counts; \texttt{cost\_high} is the upper-bound investigation cost at USD~70 per alert. Source: cost-of-misclassification verification script in the public repository.}
...
\begin{itemize}
    ...
    \item \textbf{OIII (Benchmark)} - the six-condition matrix is reported in Chapter~\ref{ch:results}; the per-round JSON archive at git tag \texttt{benchmark-2026-04-02} is reproducible from the figure-regeneration scripts in the public repository.
    ...
    \item \textbf{OV (Evidence)} - the headline numbers for FLoRA-with-RAG (peak F1 score $0.805$ within $0.05$ of the centralised upper bound, $882\times$ bandwidth reduction, MIA AUC in the $0.44$-$0.45$ band) are reported in Sections~\ref{sec:results:utility}, \ref{sec:results:efficiency}, and~\ref{sec:results:privacy}, and re-derived independently by the verification scripts in the public repository. The radar chart in Figure~\ref{fig:concept-objectives-radar} summarises the achievement against the five objectives at a glance.
\end{itemize}

\section{Future Work}
\label{sec:concl:future}

Three extensions follow naturally. First, the adapter rank was held at $r=8$ (\texttt{q\_proj}+\texttt{v\_proj}) throughout; re-benchmarking at $r=16$ with the full attention block would sharpen the utility-versus-bandwidth trade-off, since the larger adapter ($\sim$170~MB per round) remains nearly two orders of magnitude below the McMahan reference. Second, the membership inference probe is a threshold-based proxy; replacing it with a shadow-model ensemble following Shokri et al.\ would give a more conservative privacy upper bound. Third, the evaluation is single-seed; multi-seed reporting would turn the headline numbers from point estimates into confidence intervals.
